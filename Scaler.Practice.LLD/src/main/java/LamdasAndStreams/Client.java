...
//
        List<String> ls = Arrays.asList("abc","def","g","jkl");
//        List<Integer> s = ls.stream().map(String :: length).collect(Collectors.toList());
        String s1 = ls.stream().reduce((a,b)-> a + b).toString();
        System.out.println(s1);
        boolean s2 = ls.stream().anyMatch(a->a.length() < 2);
        System.out.println(s2);
        Optional<String> s3 = ls.stream().findFirst();
        System.out.println(s3);
        Optional<String> s4 = ls.stream().findAny();
        System.out.println(s4);

        String[] s5 = ls.stream().toArray(String[]::new);
        System.out.println(s5);

        List<String> s6 = ls.stream().sorted((a,b)->b.length()-a.length()).collect(Collectors.toList());
